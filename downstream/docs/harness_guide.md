...
- task workspace에 `phase_status.md`가 있으면 현재 gate와 허용 write-set을 그 파일 기준으로 먼저 확인하고, 필요하면 `downstream/scripts/validate_phase_gate.py`로 hard-stop 위반 여부를 본다. 인자 없이 실행한 기본 모드는 현재 task workspace와 `phase_status.md`의 허용/잠금 패턴에 걸리는 dirty path만 검사하고, repo 전체 dirty path까지 포함하려면 `--git-scope repo`를 명시한다.
