...
- 새 프로젝트 또는 거의 빈 프로젝트에서 greenfield 상세 설명이 더 필요할 때만 `bootstrap/docs/project_overlay/first_success_guide.md`를 본다.
- 로컬에서 어떤 명령을 먼저 돌리고 dry-run 출력을 어떻게 읽을지 막힐 때만 `bootstrap/docs/project_overlay/local_diagnostics_and_dry_run.md`를 본다.

## Overlay Validator

- unresolved placeholder 검증 규칙과 실행 명령은 `bootstrap/docs/project_overlay/unresolved_decision_validator.md`를 본다.
- 같은 프로젝트라도 `first-success`와 `phase2` readiness에서 허용되는 미결정 범위가 다르다.
- 문서 간 교차 정합성 검사는 `bootstrap/docs/project_overlay/cross_document_consistency_checker.md`를 본다.
- 기존 프로젝트 상태를 bootstrap 기준과 비교하는 read-only adopt 흐름은 `bootstrap/docs/project_overlay/adopt_dry_run.md`를 본다.
- 기존 프로젝트에서 missing file create와 explicit path overwrite만 허용하는 제한적 write 흐름은 `bootstrap/docs/project_overlay/adopt_safe_write.md`를 본다.
...
- 이 저장소에서는 먼저 `bootstrap/docs/quickstart.md`를 보고, 필요할 때만 `bootstrap/docs/project_overlay/first_success_guide.md`, `bootstrap/docs/project_overlay/local_diagnostics_and_dry_run.md`, `bootstrap/docs/how_harness_kit_works.md`로 이어진다.
...
- `AGENTS.md`
  ...
  - 실제 규칙은 `docs/entrypoint.md`로 연결하고, adapter별 파일은 이 파일만 다시 가리킨다.
...
- `docs/project/standards/testing_profile.md`
  ...
  - 테스트 실행 명령 자체와 게이트 강제 여부는 `docs/project/standards/quality_gate_profile.md`에서 정의한다.
- `docs/project/standards/commit_rule.md`
...
- 사람은 이 파일에서 process guide와 프로젝트 전용 overlay 문서를 함께 찾는다.
...
- `bootstrap/language_conventions/`
  ...
  - 실제 프로젝트에 적용할 때는 필요한 언어 파일만 골라 `docs/project/standards/coding_conventions_project.md`에 병합하거나, 별도 언어 문서로 복사한 뒤 프로젝트 문서에서 참조한다.
...
- `bootstrap/docs/project_overlay/decisions_index_template.md`
  - downstream 프로젝트의 `docs/project/decisions/README.md`로 생성하는 기본 index template다.
