...
greenfield final install 이후의 runtime 예시는 downstream 프로젝트 루트의 `docs/*`와 `scripts/*` canonical path를 기준으로 설명한다. `vendor/harness-kit/`는 delivery 또는 install-time 입력일 수 있지만 final runtime command surface는 아니다.
...
- 이 helper는 canonical bundle 재생성, 임시 install-time bundle 실행, root `scripts/*` runtime validator materialization을 한 번에 묶는다.
- 기본 legacy cleanup 경로는 `vendor/harness-kit`이고, 다른 기존 배치 경로를 정리하려면 `--vendor-path third_party/harness-kit`처럼 project-root relative path를 준다.
- legacy vendored path는 downstream `docs/` 트리 밖에 두는 것을 전제로 하며, 이미 vendored bundle이 있으면 `--force-vendor`, bootstrap 결과물이 이미 있으면 `--force-bootstrap`을 사용한다.
...
- [`downstream/docs/harness_guide.md`](downstream/docs/harness_guide.md) - reusable core guide
  - kit의 공통 진입점이다.
...
- 사용 프로젝트용 문서
  ...
  - 주 문서: [`downstream/docs/harness_guide.md`](downstream/docs/harness_guide.md), `downstream/docs/harness/common/*`, `downstream/docs/phase_*`
