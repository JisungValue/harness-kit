...
- 예: `dist/harness-kit-project-bundle/`
- 예: bundle manifest, checksum, release zip/tarball
- 예: bundle 전용 `README.md` 또는 entry 문서

이 범주의 자산은 추후 bundle generation 이슈에서 구체화한다.

## 현재 저장소 기준 기대 bundle 구조

아직 bundle generation 명령은 없지만, downstream 배포물은 최소한 아래 구조를 목표로 한다.
