...
- 기대 결과:
  ...
  - `docs/harness_guide.md`가 vendored kit guide 경로를 가리키고, `coding_conventions_project.md`가 선택 언어와 bootstrap 기준 문서를 함께 보여준다.
...
- 검증 항목: init CLI 경로가 first success 문서 세트를 생성하는지 확인
  ...
  - 결과: init CLI가 최소 문서 세트를 만들고 첫 검증 명령이 성공했으며, `docs/harness_guide.md`의 vendored guide 경로와 `coding_conventions_project.md`의 언어/bootstrap 기준 문서도 함께 확인됐다
  - 실패 또는 미실행 사유: 없음
  - 판정: 정합
...
- 검증 항목: 수동 복사 경로가 같은 최소 문서 세트로 수렴하는지 확인
  ...
  - 결과: 수동 경로도 최소 문서 세트, 첫 검증 명령 결과, vendored path 현지화, 선택 언어와 bootstrap 기준 문서, 핵심 규칙 범주 식별 신호가 함께 확인됐다
  - 실패 또는 미실행 사유: 없음
  - 판정: 정합
...
- 검증 항목: first success 가이드의 검증 명령과 expected success 상태 일치 여부 확인
  ...
  - 실행 방법 또는 확인 방식: 가이드의 검증 명령을 테스트 상수와 동일한 내용으로 실행하고, 같은 테스트에서 vendored guide 경로, `docs/standard/*` project 문서 링크, 선택 언어, bootstrap 기준 문서, 핵심 규칙 범주, profile 문서 비어 있지 않음, quality gate/commit rule의 미해결 decision marker 존재를 함께 확인
  - 결과: 가이드의 첫 검증 명령과 실제 smoke test가 같은 성공 신호를 사용하고, 가이드가 말하는 핵심 first-success content signal도 smoke test 범위 안에 포함됐다
  - 실패 또는 미실행 사유: 없음
  - 판정: 정합
...
- 최소 문서 세트와 첫 검증 명령은 실제 smoke test로 뒷받침된다.
