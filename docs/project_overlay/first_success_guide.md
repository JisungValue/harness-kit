...
- 로컬 `docs/harness_guide.md`가 공통 kit 문서와 프로젝트 전용 문서를 함께 가리킨다.
...
4. `docs/harness_guide.md`와 `docs/standard/coding_conventions_project.md` 안의 vendored 경로를 실제 배치 경로와 맞춘다.
...
vendor/harness-kit/docs/project_overlay/project_harness_guide_template.md -> docs/harness_guide.md
...
4. `docs/harness_guide.md` 안의 vendored 경로가 실제 배치 경로와 맞는지 확인한다.
...
- `AGENTS.md`가 `docs/harness_guide.md`를 우선 읽을 문서로 가리킨다.
...
- 대상 경로에 이미 `docs/harness_guide.md`나 `docs/standard/*` 파일이 있어 init CLI가 fail-fast로 중단됨
...
- consistency checker가 entrypoint 관련으로 실패하면: `AGENTS.md -> docs/harness_guide.md`, `CLAUDE.md`/`GEMINI.md` -> `AGENTS.md` 연결을 먼저 확인한다.
