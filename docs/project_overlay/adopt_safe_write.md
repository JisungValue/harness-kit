...
- `legacy entrypoint migration candidates`
  ...
  - `--migrate-legacy-entrypoint`를 주면 legacy `docs/harness_guide.md`를 `docs/project_entrypoint.md`로 rename하고, 안전한 경우 필요한 runtime entrypoint follow-up도 함께 적용한다.
...
- `migrated legacy entrypoints`
  - legacy `docs/harness_guide.md`를 새 canonical `docs/project_entrypoint.md`로 옮긴 수다.
...
2. `legacy entrypoint migration candidates`가 보이면 `docs/harness_guide.md -> docs/project_entrypoint.md` rename migration이 필요한 상태로 읽는다.
...
- canonical `docs/project_entrypoint.md`가 이미 존재하거나, `AGENTS.md`가 legacy path를 가리키는 custom 문서로 크게 diverge한 경우는 수동 검토가 먼저다.
