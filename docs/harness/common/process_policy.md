...
- 공통 권장 레이어 진행 축은 `도메인 규칙 -> 유스케이스 조합 -> 외부 연동 어댑터 -> 진입 인터페이스 -> 실행 조립`이다.
- 프로젝트가 `domain -> application -> adapter -> entrypoint -> bootstrap` 명칭을 사용한다면 같은 진행 축으로 본다.
- 프로젝트 구조에서 일부 레이어가 없거나 통합된 경우에는 존재하는 레이어만 같은 방향으로 적용한다.
- `adapter`는 필요 시 `outbound`, `persistence`, `external client` 같은 경계별 단위로 세분화할 수 있다.
- `adapter`를 세분화하면 의존성 변화가 작은 단위부터 순차적으로 진행한다.
