...
- 현재 Phase 감사가 승인 가능이 되기 전에는 다음 Phase로 이동하지 않는다.
...
- 각 Phase는 해당 `implementation.md`를 기준으로 수행한 뒤 `audit.md`를 기준으로 감사를 수행한다.
...
- 다음 Phase로 이동하려면 현재 Phase 감사가 승인 가능이어야 한다.
...
- 모든 선택된 레이어 구현이 끝나면 Phase 2 전체 감사를 1회 수행한다.
