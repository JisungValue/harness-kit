...
source repo canonical path는 `downstream/docs/*`, `bootstrap/docs/*`, `bootstrap/docs/project_overlay/*`, `bootstrap/scripts/*`이지만, generated bundle에서는 consumer 시작 경로를 유지하기 위해 downstream/bootstrap docs와 overlay 자산은 `docs/*` 아래로, bootstrap/adoption 스크립트는 `scripts/*` 아래로 materialize 한다.
