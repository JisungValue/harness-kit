...
source repo canonical path는 `bootstrap/docs/*`과 `bootstrap/docs/project_overlay/*` guide이지만, generated bundle에서는 consumer 시작 경로를 유지하기 위해 bootstrap entry docs와 overlay guide를 계속 `docs/*` 아래로 materialize 한다.
...
`docs/project_entrypoint.md`와 `docs/decisions/README.md`는 bundle 안에 직접 들어가는 파일이 아니라, downstream 프로젝트에서 각각 `docs/project_overlay/project_entrypoint_template.md`, `docs/project_overlay/decisions_index_template.md`를 bootstrap 또는 수동 복사해 생성하는 consumer-local 파일이다. 반면 source repo 기준 `bootstrap/docs/project_overlay/*.md` guide는 generated bundle 안에서 `docs/project_overlay/*.md`로 materialize 되고, `docs/project_overlay/harness_doc_guard_workflow_template.yml`은 first-success 이후 consumer project가 `.github/workflows/harness-doc-guard.yml`로 복사해 future-session CI guardrail을 고정하는 shipped asset이다.
