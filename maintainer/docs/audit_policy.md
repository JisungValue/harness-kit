...
- 개별 프로젝트의 task/phase 운영은 프로젝트 로컬 `docs/entrypoint.md`와 project overlay 문서가 담당한다.
...
- 비대상: downstream 프로젝트 로컬 `docs/entrypoint.md`, `docs/project/standards/*`, 프로젝트 저장소 안의 task 산출물 변경
...
- 프로젝트 영향 문서: `downstream/docs/harness_guide.md`, `downstream/docs/harness/common/*`, `downstream/docs/phase_*`, `bootstrap/docs/project_overlay/*`, `downstream/docs/harness/common/coding_guidelines_policy.md`, `downstream/docs/templates/task/*`, `downstream/docs/examples/*`, `bootstrap/*`, `bootstrap/scripts/bootstrap_init.py`
...
- downstream final install layout, install-time only lifecycle, root runtime command surface를 바꾸는 변경이라면 `maintainer/docs/downstream_final_layout_contract.md`의 final path와 asset taxonomy를 먼저 갱신하거나 준수했는가
...
- `README.md`, `downstream/docs/harness_guide.md`, downstream `docs/entrypoint.md` 설명, phase-local 문서 사이에 충돌이 없는가
...
- `downstream/docs/harness/common/coding_guidelines_policy.md`의 공통 품질 기준과 project convention 참조 구조가 phase 문서, template, example, bootstrap 자산 전반에서 같은 방식으로 유지되는가
...
- greenfield final install 결과를 바꾸는 변경이면 delivery bundle 경계와 final installed runtime surface를 혼동하지 않고 `docs/entrypoint.md`, `docs/project/*`, `docs/process/*`, root `scripts/*` 계약으로 설명했는가
...
- 공통 품질 기준(`downstream/docs/harness/common/coding_guidelines_policy.md`) 또는 project convention 참조 구조가 phase 문서, template, example, bootstrap 자산 사이에서 불일치함
